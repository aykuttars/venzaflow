...
@Component({
  ...
  template: `
    <div class="perm-picker" [class.perm-picker--loading]="loading">
      ...
      <div class="perm-picker__state perm-picker__state--error">{{ 'permissions.loadError' | translate }}</div>
      } @else if (groups.length === 0) {
      <div class="perm-picker__state">{{ 'permissions.empty' | translate }}</div>
      } @else {
      @for (g of groups; track g.module) {
      <section class="perm-group">
        <h4 class="perm-group__title">
          {{ moduleLabel(g.module) }}
          <span class="perm-group__count">{{ g.items.length }} {{ 'permissions.count' | translate }}</span>
        </h4>
        <ul class="perm-list">
          ...
          <li class="perm-list__item">
            ...
            >
              <span class="perm-list__label">
                ...
                <span class="perm-list__name">{{ permissionLabel(p.codename) }}</span>
              </span>
            </mat-checkbox>
          </li>
          }
  ...
  styles: [
    `
      ...
      .perm-picker--loading { min-height: 120px; display: flex; align-items: center; justify-content: center; }
      .perm-picker__state {
        display: flex; flex-direction: column; align-items: center; justify-content: center;
        gap: 12px; padding: 24px 16px; color: rgba(0, 0, 0, 0.6); font-size: 14px; text-align: center;
      }
      .perm-picker__state--error { color: #b00020; }
      .perm-group { margin: 0; padding: 0; border-bottom: 1px solid rgba(0, 0, 0, 0.08); }
      ...
      .perm-group__count {
        font-weight: 500; font-size: 11px; padding: 2px 6px; border-radius: 10px;
        background: rgba(63, 81, 181, 0.12); color: #3f51b5;
      }
      .perm-list { list-style: none; margin: 0; padding: 4px 8px 8px; }
      .perm-list__item ::ng-deep .mdc-form-field { width: 100%; align-items: flex-start; }
      .perm-list__item ::ng-deep .mdc-label {
        width: 100%; padding: 6px 4px; white-space: normal; line-height: 1.35; color: rgba(0, 0, 0, 0.87);
      }
      .perm-list__label { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
      .perm-list__code { font-family: monospace; font-size: 12px; font-weight: 500; word-break: break-all; }
      .perm-list__name { font-size: 12px; color: rgba(0, 0, 0, 0.55); }
    `,
  ],
})
export class PermissionPickerComponent implements OnInit {
  ...
  moduleLabel(module: string): string {
    return this.moduleLabelService.label(module);
  }

  permissionLabel(codename: string): string {
    const key = `permissions.${codename}`;
    const t = this.translate.instant(key);
    return t !== key ? t : codename;
  }
