#!/usr/bin/env bash
# Push and cosign e-barcode installers to the OCI artifact registry.
set -euo pipefail
...
push_installer() {
  ...
  oras push "$ref" \
    ...
    --annotation "org.opencontainers.image.title=Venzaflow e-barcode" \
    ...
    --annotation "org.opencontainers.image.description=Venzaflow e-barcode — minimum: ${MIN_OS_LABEL} (Electron 39)" \
    --annotation "org.opencontainers.image.os=${OCI_OS}" \
    --annotation "org.opencontainers.image.arch=${OCI_ARCH}" \
    --annotation "com.venzaflow.ebarcode.min-os=${MIN_OS_LABEL}" \
