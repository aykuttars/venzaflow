"""Resolve latest released e-barcode installers from the private OCI artifact registry."""
...
APP_VERSION_IN_TAG = re.compile(r"venzaflow-ebarcode-(\d+\.\d+\.\d+)", re.I)
...
def fetch_tags(*, session: requests.Session | None = None) -> list[str]:
    ...
    response = client.get(
        f"{_registry_url()}/v2/{settings.ARTIFACT_REGISTRY_EBARCODE_REPO}/tags/list",
        auth=_auth(),
        timeout=30,
    )
...
def resolve_manifest(client: requests.Session, tag: str) -> tuple[dict[str, Any], requests.Response]:
    response = client.get(
        f"{_registry_url()}/v2/{settings.ARTIFACT_REGISTRY_EBARCODE_REPO}/manifests/{tag}",
    ...
    if "index" in content_type or manifest.get("manifests"):
        for entry in manifest.get("manifests", []):
            ...
            ref_response = client.get(
                f"{_registry_url()}/v2/{settings.ARTIFACT_REGISTRY_EBARCODE_REPO}/manifests/{digest}",
                headers={"Accept": MANIFEST_ACCEPT},
                auth=_auth(),
                timeout=60,
            )
...
def stream_blob(digest: str):
    with requests.get(
        f"{_registry_url()}/v2/{settings.ARTIFACT_REGISTRY_EBARCODE_REPO}/blobs/{digest}",
        auth=_auth(),
        stream=True,
        timeout=120,
    ) as response:
...
def build_releases_payload(*, download_url_builder) -> dict[str, Any]:
    ...
    cache_key = "barcode:ebarcode:releases:v1"
