...
| Clinic Cashier | `billing.{read,write}`, `patients.read`, `oral.read`, `dashboard.read` |
| Accounting | `accounting.{read,write}`, `billing.read`, `dashboard.read` |
| Security | `audit.read`, `dashboard.read`, `settings.read` |
| Doctor | `patients.{read,write}`, `oral.{read,write}`, `appointments.{read,write}`, `customers.read`, `dashboard.read` |
| Dentist | `patients.{read,write}`, `oral.{read,write}`, `appointments.{read,write}`, `dashboard.read` |
